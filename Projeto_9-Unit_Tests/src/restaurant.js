...
function getValueOfFetchMenu(elementConsumption) {
  const valuesFetchMenu = Object.values(objetoRetornado.fetchMenu());
  for (const values of valuesFetchMenu) {
    if (values[elementConsumption] !== undefined) {
      return values[elementConsumption];
    }
  }
}
...
function payFunction() {
  ...
  for (const elementConsumption of objetoRetornado.consumption) {
    somaDosPreçosDosPedidos += getValueOfFetchMenu(elementConsumption);
  }
  somaDosPreçosDosPedidos += somaDosPreçosDosPedidos * 0.1;
